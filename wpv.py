...
class Stack:
    ...
    def get_visible_light_transmission(self,lamrange,inc_angle):
        ...
        numerator = integ(lambda lam: self.Is(lam)*self.cieplf(lam)*self.get_RAT(lam,inc_angle)[2], nitn=10, neval=300)[0]
        denominator = integ(lambda lam: self.Is(lam)*self.cieplf(lam), nitn=10, neval=300)[0]
